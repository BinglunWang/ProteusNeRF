...
def calculate_clip_score_each(dataloader, model, real_flag, fake_flag):
    score_list = []
    logit_scale = model.logit_scale.exp()
    for batch_data in tqdm(dataloader):
        real = batch_data['real']
        ...
        # score = logit_scale * real_features @ fake_features.t()
        # score_acc += torch.diag(score).sum()
        # score = logit_scale * (fake_features * real_features).detach().cpu()
        # calculate cos similarity for features
        cos_simlarity = cos(real_features, fake_features).detach().cpu()
        score_list.append(cos_simlarity)
